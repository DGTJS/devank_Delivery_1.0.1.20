...
class OrderTrackingScreenState extends State<OrderTrackingScreen>
  ...
  StreamSubscription<OrderModel?>? _locationSubscription;
  ...
  void initState() {
    ...
    // Inscrição no stream de localização
    _locationSubscription =
        orderController!.deliveryLocationStream.listen((OrderModel? order) {
      if (order?.deliveryMan != null) {
        _updateDeliveryManMarker(order!.deliveryMan!);
      }
    });
  }

  // Atualiza o marcador do entregador no mapa
  void _updateDeliveryManMarker(DeliveryMan deliveryMan)  async {
    final deliveryManLatLng =
    ...
    setState(() {
      // Remove o marcador antigo do entregador
      _markers.removeWhere((marker) => marker.markerId.value == 'delivery_boy');
      // Adiciona um novo marcador na localização atualizada
      _markers.add(
            Marker(
    ...
    // Move a câmera para a nova posição do entregador
    _controller?.animateCamera(CameraUpdate.newLatLng(deliveryManLatLng));
  }
                      ...
                      background: Stack(children: [
                        GoogleMap(
                          ...
                          onMapCreated: (GoogleMapController controller) {
                            ...
                            // Se já houver uma localização do entregador, adiciona o marcador inicial
                            if (track?.deliveryMan != null) {
                              _updateDeliveryManMarker(track!.deliveryMan!);
                            }
                          },
